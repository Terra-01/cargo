...
function Thumb({ id, fallback }: { id: string; fallback: string }) {
  return (
    <span className="sg-swatch__img" style={{ background: fallback }}>
      {/* Plain <img> is correct here, not next/image: this is a tiny,
          decorative (aria-hidden), already-optimized static webp generated by
          the project's own `npm run thumbnails` script. next/image's loader
          adds no optimization for an already-sized 1x asset and would
          complicate the onError graceful-degradation to the CSS swatch when a
          thumbnail is missing (404). */}
